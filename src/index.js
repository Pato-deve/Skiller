...
import { getSkillsFromDb, installSkill, Skill } from './core/skills.js';
const SKILLER_ASCII = `
...
async function main() {
    ...
    const handleGlobalQuit = (str, key) => {
    ...
    const filteredSkills = allSkills.filter(skill => targetCategories.has(skill.category || '') ||
        skill.id.includes('git') || // Siempre útil
        detectedTags.some(tag => skill.id.includes(tag) || skill.description.toLowerCase().includes(tag))).slice(0, 50); // Limitamos a 50 para no romper la TUI por ahora
...
function renderCustomPrompt(options, detectedTags) {
    return new Promise((resolve) => {
        ...
        const render = () => {
            ...
            options.forEach((opt, i) => {
                if (i < scrollOffset || i >= scrollOffset + PAGE_SIZE)
                    return;
